...
void PC::depositMythikin(Mythikin& mover){
    if(user->getTeam().getSize() > 1) { // Only able to deposit as long as there is more than one Mythikin in the team.
...
void PC::withdrawMythikin(const string name){
    if(user->getTeam().getSize() <= user->getTeam().getMaxSize()){ // Only able to withdraw as long as the team is not full.
